...
import { FileText, DollarSign, PackageOpen, File } from 'lucide-react';
  ...
  return (
    <div className="file-table-container">
      <div className="table-header">
        <h3>
          <FileText />
          File Analysis ({sortedFiles.length} files)
        </h3>
        ...
        <div className="table-controls">
          <CustomSelect
            ...
            options={[
              ...
              { value: 'old', label: `Old Files Only (${files.filter(f => f.analysis?.isOld).length})` },
              { value: 'duplicates', label: `Duplicates Only (${files.filter(f => f.analysis?.isDuplicate).length})` },
              { value: 'savings', label: `With Savings (${files.filter(f => (f.analysis?.savingsPerMonth || 0) > 0).length})` }
            ]}
          />
